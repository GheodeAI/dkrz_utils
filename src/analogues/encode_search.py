...
def train_base_model(model_enc, model_dec, year_start_str, year_end_str, X_train, idx=0, hw="params.json", model_type=""):
    ...
    model_enc.save(f"./models/{hw_name}/MvAE_{hw_name}_{model_type}_{year_start_str}-{year_end_str}.h5")
    model_dec.save(f"./models/{hw_name}/MvAE_{hw_name}_{model_type}_{year_start_str}-{year_end_str}_dec_.h5")

    return autoencoder

def load_data_past2k(year_start, year_end, year_part, year_part_end, idx=0, hw="params.json", out_preprocess=["x_train_pre_pred", "x_test_pre_pred"]):
    with open(hw) as f:
        params = json.load(f)
    ...
    params["target_dataset"] = params["target_dataset"].replace("7000", str(year_start)).replace("7099", str(year_end))
    params["pred_dataset"] = params["pred_dataset"].replace("7000", str(year_start)).replace("7099", str(year_end))
    params["ident_dataset"] = params["ident_dataset"].replace("7000", str(year_start)).replace("7099", str(year_end))
...
def load_data_ecmwf(year_start, year_end, hw="params.json", out_preprocess=["time_indust_pred", "indust_target", "img_size"], params=None):
    if params is None:
        with open(hw) as f:
            params = json.load(f)
    ...
    params['post_init'] = pd.Timestamp(str(year_start) + params['post_init'][4:] +' 00:00:00')
    params['post_end'] = pd.Timestamp(str(year_end) + params['post_end'][4:] +' 06:00:00')
    if type(params["data_of_interest_init"]) != pd.Timestamp:
        params["data_of_interest_init"] = pd.Timestamp(params["data_of_interest_init"] +' 00:00:00')
    if type(params["data_of_interest_end"]) != pd.Timestamp:
        params["data_of_interest_end"] = pd.Timestamp(params["data_of_interest_end"] +' 06:00:00')
...
def load_data_era5(year_start, year_end, hw="params.json", out_preprocess=["indust_pred", "data_of_interest_pred", "time_indust_pred", "indust_target", "img_size"], params=None):
    if params is None:
        with open(hw) as f:
            params = json.load(f)
    params["name"] = params["name"].replace("#", "0")
...
def train_all_past2k(model_enc, model_dec, year_range=(0, 1850), hw="params.json", model_type=""):
    for i, year_start in enumerate(np.arange(7000, 8850, 100)):
        ...
        for j, year_part in enumerate(np.arange(year_start, year_end, 25)):
            ...
            X_train, _ = load_data_past2k(year_start, year_end, year_part, year_part_end, idx, hw)
            X_train = np.nan_to_num(X_train)
            train_base_model(model_enc, model_dec, year_part - 7000, year_part_end - 7000, X_train, idx, hw, model_type)
            print(f"trained on past2k up to year {year_end}.", flush=True)
    return model_enc, model_dec

def train_era5(model_enc, model_dec, year_range=(1940, 2022), hw="params.json", model_type=""):
    X_train = load_data_era5(year_range[0], year_range[1], hw, out_preprocess=["indust_pred"])[0]
    print(f'\nX_train shape: {np.shape(X_train)}\n')
    X_train = np.nan_to_num(X_train)
    train_base_model(model_enc, model_dec, year_range[0], year_range[1], X_train, 0, hw, model_type)
...
def encode_past2k(hw, model_type="", year_range=(0, 1850)):
    ...
    for i, year_start in enumerate(np.arange(year_range[0], year_range[1], 100)):
        ...
        for j, year_part in enumerate(np.arange(year_start, year_end, 25)):
            ...
            print(f'year_part: {year_part}', flush=True)
            print(f'year_part_end: {year_part_end}', flush=True)
            AE_ind = keras.models.load_model(f"./models/{hw_name}/MvAE_{hw_name}_{model_type}_{str(year_part)}-{str(year_part_end)}.h5", custom_objects={'keras': keras,'AutoEncoders': AutoEncoders})
            encode_era5(AE_ind, year_part_range=(year_part, year_part_end), hw=hw, model_type="past2k", idx=4*i+j)
    return
...
def encode_ecmwf(AE_ind, year_range=(1993, 2016), hw="params.json"):
    ...
    for i in range(1,26):
        ...
        indust_pred = load_data_era5(year_range[0], year_range[1], hw, ["indust_pred"], params=params_i)
        indust_pred_encoded = AE_ind.predict(indust_pred)
        print(f"encoded on ecmwf for ens {i:02d}.", flush=True)
        del indust_pred
        np.save(f"./data/encoded/indust_pred_encoded_ens{i:02d}_{year_range[0]}-{year_range[1]}_{hw_name}.npy", indust_pred_encoded)
    return
    

def encode_era5(AE_ind, year_range=(1940, 2022), year_part_range=(1940, 2022), hw="params.json", model_type="", idx=0):
    hw_name = hw.split('.')[0].split('/')[-1]
    if model_type=="past2k":
        indust_pred = load_data_past2k(year_range[0], year_range[1], year_part_range[0], year_part_range[1], idx=idx, hw=hw, out_preprocess=["indust_pred"])
    else:
    ...
    del indust_pred
    np.save(f"./data/encoded/indust_pred_encoded_{year_part_range[0]}-{year_part_range[1]}_{hw_name}.npy", indust_pred_encoded)
    del indust_pred_encoded
    if model_type=="past2k":
        data_of_interest_pred = load_data_past2k(year_range[0], year_range[1], year_part_range[0], year_part_range[1], idx=idx, hw=hw, out_preprocess=["data_of_interest_pred"])
    else:
        data_of_interest_pred = load_data_era5(year_range[0], year_range[1], hw, ["data_of_interest_pred"])
    ...
    np.save(f"./data/encoded/data_of_interest_pred_encoded_ind_{year_part_range[0]}-{year_part_range[1]}_{hw_name}.npy", data_of_interest_pred_encoded_ind)
    del data_of_interest_pred_encoded_ind
    return


def search_ecmwf(year_range=(1993, 2016), year_part_range=(1993, 2016), hw="params.json"):
    hw_name = hw.split('.')[0].split('/')[-1]
    with open(hw) as f:
        params = json.load(f)
    ...
    if params["interest_region_type"] == "coord":
        int_reg = calculate_interest_region(params["interest_region"], [params["latitude_min"], params["latitude_max"], params["longitude_min"], params["longitude_max"]], params["resolution"], False, "file")
    ...
    print(f'Heatwave period: {heatwave_period}\n {np.shape(heatwave_period)}\n', flush=True)
    ...
    _, _, time_indust_pred_gen, indust_target_gen, img_size = load_data_era5(year_range[0], year_range[1], hw, params=params)
    for idx, init in enumerate(tqdm(params_multiple["data_of_interest_init"])):
        ...
        min_time = params["data_of_interest_init"] - datetime.timedelta(days=int(min_days_time))
        max_time = params["data_of_interest_init"].replace(hour=6) + datetime.timedelta(days=int(max_days_time))
        
        # Create boolean mask for the time range
        time_mask = (time_indust_pred_gen.time >= min_time) & (time_indust_pred_gen.time <= max_time)
        ...
        for i in tqdm(range(1,26), desc="Processing ens"):
            # AE Post
            file_time_name = f'./comparison-csv/analogues-ae-am-post-{params["season"]}{params["name"]}_ens{i:02d}_x{params["iter"]}-{params["data_of_interest_init"]}-epoch{params["n_epochs"]}-latent{params["latent_dim"]}-k{params["k"]}-arch{params["arch"]}-{"VAE" if params["use_VAE"] else "noVAE"}_{year_part_range[0]}-{year_part_range[1]}_{current.year}-{current.month}-{current.day}-{current.hour}-{current.minute}-{current.second}.npy'.replace(" ","").replace("'", "").replace(",","")
            ## Load data
            indust_pred_encoded = np.load(f"./data/encoded/indust_pred_encoded_ens{i:02d}_{year_part_range[0]}-{year_part_range[1]}_{hw_name}.npy")
            ...
            data_of_interest_pred_encoded_ind = np.load(f"./data/encoded/indust_pred_encoded_{year_part_range[0]}-{year_part_range[1]}_{hw_name[:-5]}era5.npy")
            data_of_interest_pred_encoded_ind = data_of_interest_pred_encoded_ind[idx]
            print(f"shape data_of_interest_pred_encoded_ind: {np.shape(data_of_interest_pred_encoded_ind)}")
            latent_analog_ind = analogSearch(params["p"], params["k"], indust_pred_encoded, data_of_interest_pred_encoded_ind, time_indust_pred, indust_target, False, threshold=0, img_size=img_size, iter=params["iter"], replace_choice=params["replace_choice"], target_var_name=params["target_var_name"], file_time_name=file_time_name)
            ...
            for j in range(params["iter"]):
                ...
                dict_stats[f'Analog{j}'] = [f"{latent_analog_ind[3][j]:.10f}", str(latent_analog_ind[2][j].data)[:10], str(latent_analog_ind[2][j].data)[11:13]]
            del data_of_interest_pred_encoded_ind
            df_stats = pd.DataFrame.from_dict(dict_stats,orient='index',columns=['pred-diff','time','leadtime'])
...
def search_past2k(year_range=(1940, 2022), year_part_range=(0, 1885), hw="params.json"):
    hw_name = hw.split('.')[0].split('/')[-1]
    for i, year_start in enumerate(np.arange(year_part_range[0], year_part_range[1], 100)):
        idx = i + 1
        year_end = year_start + 99
        if year_start == 1800:
            year_end = 1850
        ...
        for j, year_part in enumerate(np.arange(year_start, year_end, 25)):
            ...
            print(f'year_part: {year_part}', flush=True)
            print(f'year_part_end: {year_part_end}', flush=True)
            search_era5(year_range=year_range, year_part_range=(year_part,year_part_end),hw=hw)
    return

def search_era5(year_range=(1940, 2022), year_part_range=(1940, 2022), hw="params.json"):
    hw_name = hw.split('.')[0].split('/')[-1]
    with open(hw) as f:
        params = json.load(f)
    ...
    for idx, init in enumerate(params_multiple["data_of_interest_init"]):
        ...
        # Analog Post
        file_time_name = f'./comparison-csv/analogues-am-post-{params["season"]}{params["name"]}x{params["iter"]}-{params["data_of_interest_init"]}-epoch{params["n_epochs"]}-latent{params["latent_dim"]}-k{params["k"]}-arch{params["arch"]}-{"VAE" if params["use_VAE"] else "noVAE"}_{year_part_range[0]}-{year_part_range[1]}_{current.year}-{current.month}-{current.day}-{current.hour}-{current.minute}-{current.second}.npy'.replace(" ","").replace("'", "").replace(",","")
        ## Load data
        indust_pred, data_of_interest_pred, time_indust_pred, indust_target, img_size = load_data_era5(year_range[0], year_range[1], hw, params=params)
        print(f'\nShape in encode_seach: {np.shape(data_of_interest_pred)}\n', flush=True)
        ## Search
        analog_ind = analogSearch(params["p"], params["k"], indust_pred, data_of_interest_pred, time_indust_pred, indust_target, False, threshold=0, img_size=img_size, iter=params["iter"], replace_choice=params["replace_choice"], target_var_name=params["target_var_name"], file_time_name=file_time_name)
        ## Del data
        del indust_pred, data_of_interest_pred
        
        # AE Post
        file_time_name = f'./comparison-csv/analogues-ae-am-post-{params["season"]}{params["name"]}x{params["iter"]}-{params["data_of_interest_init"]}-epoch{params["n_epochs"]}-latent{params["latent_dim"]}-k{params["k"]}-arch{params["arch"]}-{"VAE" if params["use_VAE"] else "noVAE"}_{year_part_range[0]}-{year_part_range[1]}_{current.year}-{current.month}-{current.day}-{current.hour}-{current.minute}-{current.second}.npy'.replace(" ","").replace("'", "").replace(",","")
        ## Load data
        indust_pred_encoded = np.load(f"./data/encoded/indust_pred_encoded_{year_part_range[0]}-{year_part_range[1]}_{hw_name}.npy")
        data_of_interest_pred_encoded_ind = np.load(f"./data/encoded/data_of_interest_pred_encoded_ind_{year_part_range[0]}-{year_part_range[1]}_{hw_name}.npy")
        data_of_interest_pred_encoded_ind = data_of_interest_pred_encoded_ind[idx]
        latent_analog_ind = analogSearch(params["p"], params["k"], indust_pred_encoded, data_of_interest_pred_encoded_ind, time_indust_pred, indust_target, False, threshold=0, img_size=img_size, iter=params["iter"], replace_choice=params["replace_choice"], target_var_name=params["target_var_name"], file_time_name=file_time_name)
        del indust_pred_encoded, time_indust_pred, indust_target
        #del data_of_interest_pred_encoded_ind
        ...
        dict_stats = {}
        data_of_interest_pred, data_of_interest_target = load_data_era5(year_range[0], year_range[1], hw, ["data_of_interest_pred", "data_of_interest_target"], params=params)
        ...
        del data_of_interest_pred, data_of_interest_target, data_of_interest_pred_encoded_ind
...
def post_process_anal(hw="params.json"):
    with open(hw) as f:
        params = json.load(f)
    heatwave_period = np.arange(datetime.datetime.strptime(params["data_of_interest_init"], '%Y-%m-%d'), datetime.datetime.strptime(params["data_of_interest_end"], '%Y-%m-%d')+datetime.timedelta(days=1), datetime.timedelta(days=1))
    ...
    df_all_days.to_csv(f"./anal_dict/post_processed_analogues_{str(heatwave_period[0])[:4]}-{str(heatwave_period[-1])[:4]}.csv")
...
def main():
    ...
    with open(hw) as f:
        params = json.load(f)
    year_range=(int(params["post_init"].split('-')[0]), int(params["post_end"].split('-')[0]))
    if method in ["train", "train-past2k"]:
        in_shape = (
            (params["latitude_max"] - params["latitude_min"]) // params["resolution"] + 1,
            (params["longitude_max"] - params["longitude_min"]) // params["resolution"] + 1,
        )
        print(f"Input shape for {hw.split('.')[0].split('/')[-1]}: {in_shape}", flush=True)
        ae_object = va_am.utils.AutoEncoders.AE_conv(in_shape, 400, arch=10, in_channels=1, out_channels=1)
        ...
        if method == "train-past2k":
            ...
            train_all_past2k(base_model_enc, base_model_dec, year_range=(0, 1885), hw=hw, model_type="past2k")
        else:
            train_era5(base_model_enc, base_model_dec, year_range=(1940, 2022), hw=hw, model_type="era5")
        ...
        message = ValueError(f"Not recognized {method} method. The available methods are 'encode' (default), 'search' or 'train'")
        raise message
    return
