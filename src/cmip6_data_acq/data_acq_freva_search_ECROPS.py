...
def freva_search_ssp(project, model, var, freq, experiment):
    ...
    ssp_files = freva.databrowser(project=project,
                                  model=model,
                                  variable=var,
                                  time_frequency=freq,
    ...
    unique_ensembles = np.unique(np.array(all_ensembles))  # then filter out only the unique ensemble values
    logging.info(str(experiment) + " for " + str(var) + " unique ensemble ids = " + str(unique_ensembles))
    ...
    ssp_csv_filename = str(model) + "__" + project + "_" + str(experiment) + "_" + str(var) + "_" + str(freq) + ".csv"
    ssp_files_array.tofile(os.path.join(os.sep, homevardir, ssp_csv_filename), sep='\n')
    historical_csv_filename = str(model) + "__" + project + "_" + str(experiment) + "_" + str(var) + "_" + str(freq) + "_historical" + ".csv"
...
def freva_search_historical(project, model, var, freq):
    ...
    historical_files = freva.databrowser(project=project,
                                         model=model,
                                         variable=var,
                                         time_frequency=freq,
    ...
    unique_ensembles = np.unique(np.array(all_ensembles))  # then filter out only the unique ensemble values
    logging.info("Historical for " + str(var) + " unique ensemble ids = " + str(unique_ensembles))
    ...
    ## Write everything to csv files
    all_historical_csv = str(model) + "__" + project + "_" + str(var) + "_" + str(freq) + "_allhistorical" + ".csv"
...
def freva_search_reanalysis(project, experiment, var, freq):#, geopoten_value):
    ...
    reanalysis_files = freva.databrowser(project=project,
                                         time_frequency=freq,
                                         variable=var,
                                         experiment=experiment)
    ...
    for reanalysis_file in reanalysis_files_array:
        ...
        all_ensembles.append(res.get("ensemble")[0])  # get the first(and only) value of the dictionary <ensemble:value>
    unique_ensembles = np.unique(np.array(all_ensembles))  # then filter out only the unique ensemble values
    logging.info(str(experiment) + " reanalysis for " + str(var) + " unique ensemble ids = " + str(unique_ensembles))

    ## Write everything to csv files
    all_reanalysis_csv_filename = str(experiment) + "__" + project + "_" + str(freq) + "_" + str(var) + ".csv"
...
def get_files_from_unique_ensembles(project, model, var, freq, experiment, unique_ensemble_list):
    ...
    for unique_ens in unique_ensemble_list:
        files = freva.databrowser(project=project,
                                  model=model,
                                  ensemble=unique_ens,
                                  variable=var,
                                  time_frequency=freq,
        ...
        logging.info(str(experiment) + " " + str(var) + " files for ensemble " + str(unique_ens) + " = " + str(n))
